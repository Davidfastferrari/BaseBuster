...
contract FlashSwap is FlashLoanSimpleReceiverBase {
    ...
    address constant UNISWAP_V2_ROUTER =
        0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24; // swap exact tokens for tokens
    address constant SUSHISWAP_V2_ROUTER =
        0x6BDED42c6DA8FBf0d2bA55B2fa120C5e0c8D7891; // swap exact tokens for tokens
    address constant PANCAKESWAP_V2_ROUTER =
        0x8cFe327CEc66d1C090Dd72bd0FF11d690C33a2Eb; // swap exact tokens for tokens
    address constant BASESWAP_V2_ROUTER =
        0x327Df1E6de05895d2ab08513aaDD9313Fe505d86; // swap exact tokens for tokens

    // Default uniswapv3 router
    address constant UNISWAP_V3_ROUTER =
        0x2626664c2603336E57B271c5C0b26F421741e481; // exactInputSingle
    address constant PANCAKESWAP_V3_ROUTER =
        0x678Aa4bF4E210cf2166753e054d5b7c31cc7fa86; // exactInputSingle

    // no swapExactTokensFortokens, have exactInput
    address constant SUSHISWAP_V3_ROUTER =
        0xFB7eF66a7e61224DD6FcD0D7d9C3be5C8B049b9f; // exactInputSingke, but has the deadline param
    address constant BASESWAP_V3_ROUTER =
        0x1B8eea9315bE495187D873DA7773a874545D9D48; // exactInputSingle, but has the deadline param
    address constant SLIPSTREAM_ROUTER =
        0xBE6D8f0d05cC4be24d5167a3eF062215bE6D18a5; // exactInputSingle, but has the deadline param

    // this is swap exact tokens for tokens but has a different route, tuple instead of array, .. annoying
    address constant AERODOME_ROUTER =
        0xcF77a3Ba9A5CA399B7c97c74d54e5b1Beb874E43; // swapExactTokensFortokens, tuple
    ...
    function executeOperation(
        ...
        (SwapStep[] memory steps, address caller) = abi.decode(
            params,
            (SwapStep[], address)
        );
        ...
        for (uint i = 0; i < steps.length; i++) {
            ...
            uint256 balanceBefore = IERC20(step.tokenIn).balanceOf(
                address(this)
            );
            ...
            if (step.protocol <= 3) {
                ...
                _swapV3Deadline(
                    ...
                    balanceBefore,
                    step.fee,
                    _getV3RouterDeadline(step.protocol)
                );
            ...
            uint256 balanceAfter = IERC20(step.tokenOut).balanceOf(
                address(this)
            );
    ...
    function _swapV3Deadline(
        ...
        V3Deadline.ExactInputSingleParams memory params = V3Deadline
            .ExactInputSingleParams({
                tokenIn: tokenIn,
                tokenOut: tokenOut,
                fee: fee,
    ...
    function _swapAerodome(
        ...
        routes[0] = IAerodrome.Route({
            from: tokenIn,
            to: tokenOut,
            stable: false, // Assuming we want to use the volatile pair
            factory: address(0) // You might need to set the correct factory address here
        });
