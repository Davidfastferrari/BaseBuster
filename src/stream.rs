...
pub async fn stream_new_blocks(block_sender: Sender<Block>) {
    ...
    while let Some(block) = stream.next().await {
        ...
        info!("New block received at timestamp {}: {:?}", now, block.header.number);
        match block_sender.send(block).await {
